...
test_data <-read.csv("C:/Resources/Dissertation/Anonymisation/SVM - Working/adultgeneralisedata.csv")
# Convert the target variable to a binary factor
test_data$income <- factor(ifelse(test_data$income == ">50K", 1, 0))
# Train the SVM model
svm_model <- svm(income ~ ., data = train_data, kernel = "radial", cost = 100)
